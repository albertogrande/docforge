# Forge¹ — open-source, AI-native docs platform · v0.1 build plan

> ¹ **Name is a placeholder.** Working scope `@docforge/*`, CLI binary `forge`. Pick the real
...
| 7 | Producer mechanism | **GitHub-first.** Local MCP write-tools shell to `git`/`gh`; a `ForgeHost` interface hedges GitLab/Gitea/GitHub-App later. |
...
```
forge/  (repo root — Apache-2.0)
├─ packages/
│  ├─ schema/            @docforge/schema      SSOT content model + Zod + provenance shapes (leaf; Zod only)
│  ├─ core/              @docforge/core        renderer-agnostic engine: load/getPage/search/renderMarkdown
│  ├─ provenance/        @docforge/provenance  provenance record: read/merge/recordTransition/verify
│  ├─ gates/             @docforge/gates       validation rules (TS) + engine behind `forge check`
│  ├─ mcp/               @docforge/mcp         MCP server: read tools + WRITE tools (the moat surface)
│  ├─ producer/          @docforge/producer    draft→branch→PR→approve→state-flip; ForgeHost interface
│  ├─ cli/               @docforge/cli         the `forge` binary
│  ├─ adapter-kit/       @docforge/adapter-kit core↔adapter contract + conformance test suite
│  ├─ adapter-fumadocs/  @docforge/adapter-fumadocs   v1 reference renderer (Next/React/Fumadocs)
│  └─ actions/           @docforge/actions     composite GH Actions (gate, approval→flip, freshness)
...
├─ create-docforge/      `npm create docforge` scaffolder (v0.2)
...
|---|---|---|
| `…/developer-marketing-handbook/mcp/handbook.mjs` | `@docforge/core` | `loadPages/getPage/searchHandbook` (BM25 + `github-slugger` anchors) ported ~verbatim; add `renderMarkdown` (the "prepend H1 only if missing" canonicalizer) as the single parity source both MCP and the `.md` route call. |
| `…/scripts/check_content_quality.py` | `@docforge/gates` rules | Footnote integrity, citation discipline (`## Sources`), freshness SLA with teeth (`last_reviewed ≤ today < review_by`, overdue **fails**), reachability — each a TS rule returning structured diagnostics. |
| `…/scripts/check_frontmatter.py` | `@docforge/gates` rules | `frontmatter-required`, `enums-valid`, `dates-valid`. |
| `…/scripts/check_links.py` | `@docforge/gates` rule | `links-resolve` + `anchors-resolve`. |
| `…/scripts/check_parity.mjs` | `@docforge/adapter-kit` | Becomes the **adapter conformance suite** (md-parity, nav coverage, anchor resolution) + a `no-inline-enums` gate. |
| `…/schema/content-model.json` + `…/src/content.config.ts` | `@docforge/schema` | The SSOT-consumed-by-Zod pattern + `superRefine` boundary mechanism, generalized to a domain-neutral base model (profile mechanism deferred to v0.2). |
| `…/.claude/agents/*`, `skills/*`, `CLAUDE.md`, `.github/workflows/*` | repo `.claude/`, `@docforge/actions`, `create-docforge` templates | Generalized + extended to document the producer loop and provenance rules for agents. |

## The producer loop (v0.1)

**Mechanism = local MCP write-tools (drafting) + GitHub Action (gating & approval-recording).**
A GitHub App is the multi-tenant upgrade, pre-wired via `ForgeHost`, shipped later in `ee/`.

```
1. Agent (MCP)  draft_page({path, frontmatter, body})
   → producer writes path.md with status:draft + seeded provenance; `forge check` runs in-proc;
     diagnostics returned so the agent self-corrects BEFORE opening a PR.
2. Agent (MCP)  propose_changes({title, summary})
   → git checkout -b forge/draft/<slug>-<sha>;  commit --trailer "Forge-Provenance: …";
     git push;  gh pr create --label forge:draft.
3. CI (@docforge/actions)  forge check  → all gates + `draft-pages-not-reviewed`
...
5. on pull_request_review==approved → Action runs `forge approve`:
...
**git commit trailer** `Forge-Provenance:` on the transition commit — ties content hash to author
...
`@docforge/schema`) + a `provenance-consistency` gate (`reviewed ⇒ reviewed_by + a reviewed
...
Query via `forge prov <path>` and `forge prov --filter authored_by=ai --status reviewed` (the
dogfood site renders a `/trust` dashboard generalizing the handbook's `/system`).
`@docforge/provenance.toC2PAManifest()` ships as a **typed mapping stub** (page-level → C2PA
assertions) so the data model is attestation-ready; actual signing is the deferred commercial
feature.

## Core ↔ adapter contract (`@docforge/adapter-kit`)
...
- **Governance files:** `README`, `CONTRIBUTING` (DCO + `ee/` boundary), `CODE_OF_CONDUCT`
  ...
  dogfoods the loop: "did `forge check` pass / is this `draft` not `reviewed`?"), `GOVERNANCE`,
...
- **Agent/Claude scaffolding:** `AGENTS.md` + `CLAUDE.md` (ported, now documenting the producer
  ...
  `create-docforge` templates.
- **CI matrix:** `{os:[ubuntu,macos], node:[22,24]}` test/build; `lint`+`typecheck`; a `gates`
  job running `forge check` against `apps/docs` + `examples/minimal` (dogfooding in CI); the
...
2. **`@docforge/schema`** — domain-neutral base model + Zod `buildSchema()` + `ProvenanceSchema`
   + `LIFECYCLE_STATES`.
3. **`@docforge/core`** — port `loadPages/getPage/search` + add `renderMarkdown`, `resolveConfig`
   (`docforge.config.ts`), `nav` builder. Unit tests on the ported BM25/anchor logic.
4. **`@docforge/provenance`** — record types, `readProvenance/recordTransition/verify`,
   frontmatter writer, `toC2PAManifest` stub.
5. **`@docforge/gates`** — port all handbook rules to TS + `provenance-consistency` +
   `draft-pages-not-reviewed`; `runGates()` returning structured diagnostics.
6. **`@docforge/producer`** — `ForgeHost` interface, `LocalGitHost` + `GitHubHost` (`gh`),
   `ProducerEngine` (draft→branch→commit-with-trailer→push→PR) and `approve` (state-flip +
   provenance).
7. **`@docforge/mcp`** — read tools (ported) + write tools `draft_page`/`update_page`/
   `propose_changes`/`check`/`request_review` over `@modelcontextprotocol/sdk` (stdio).
8. **`@docforge/cli`** — `forge init/check/draft/open-pr/approve/prov/mcp` (citty).
9. **`@docforge/adapter-kit`** + **`@docforge/adapter-fumadocs`** (minimal render + `.md` route +
   nav + badge) + conformance tests.
10. **`@docforge/actions`** — gate workflow, approval→flip workflow, freshness watch.
...
1. Register the MCP server (`forge mcp` / `claude mcp add`); from Claude Code call `draft_page`
   then `propose_changes` → a `forge/draft/*` branch + a PR open with `status: draft` and a seeded
   provenance block + a `Forge-Provenance` commit trailer.
2. CI runs `forge check`: all gates pass; the `draft-pages-not-reviewed` gate confirms the PR
   does **not** self-promote to `reviewed`.
3. A human approves the PR in GitHub → the Action runs `forge approve` → frontmatter flips
   `draft→reviewed`, `last_reviewed`/`review_by` set, a `reviewed` transition + trailer appended,
   PR merged.
4. `forge prov <path>` prints the full chain (ai-authored → model → sources → human reviewer →
...
- **Repo location** — propose a sibling dir, e.g. `/Users/alberto/Documents/Code/docforge`
  (placeholder), fresh `git init`, new GitHub repo. Adjust on approval.
- **Name** — placeholder `@docforge/*` / `forge` until you pick + verify availability; rename
...
`ee/`) · `create-docforge` polish (v0.2) · MDX-components-as-data, i18n provenance, multi-repo
federation (post-1.0).
