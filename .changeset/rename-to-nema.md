...
PR label is `nema:draft`, and the `NEMA_ROOT` env var is now `NEMA_ROOT`. Internal types
`NemaConfig`/`NemaHost`/`NemaTools`/`NemaMcpServer` are renamed to the `Nema*` prefix.
